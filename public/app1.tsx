  ...
  const startSpeechRecognition = () => {
    ...
    try {
      ...
      recognition.onresult = (event: any) => {
        ...
        if (!transcript) {
          setSpeechStatus("Could not understand speech");
          return;
        }

        console.log("📝 Transcript:", transcript);
        onSend(transcript);
        setSpeechStatus(`Sent: "${transcript}"`);
      };
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event?.error === "not-allowed") {
          ...
          setSpeechStatus(`Speech error: ${event?.error || "unknown"}`);
        }
      };
  ...
  return (
    <div className="chat">
      ...
      <div className="chat__feed">
        {messages.length === 0 && <div className="chat__empty">No transmissions yet…</div>}
      ...
      {role === "mute" && (
        <div className="chat__muted-notice">
          🔇 You are mute — you cannot send messages
        </div>
      )}
    </div>
  );
    ...
    <div className="home">
      ...
      <div className="home__roles">
        ...
        <div className="hcard">
          ...
          <p className="hcard__desc">
            You have the defusal manual — but you cannot hear. Use speech input to send instructions
            to the Agent.
          </p>
        </div>
      </div>
...
function App() {
  ...
  const { send } = useGameWS((msg) => {
    switch (msg.type) {
      ...
      case "agent-text":
        console.log("✅ RECEIVED agent-text:", msg.content);

        if (msg.content) {
          addMsg("agent", msg.content);

          if (myRole === "mute") {
            console.log("🔊 SPEAKING ON MUTE SIDE:", msg.content);
